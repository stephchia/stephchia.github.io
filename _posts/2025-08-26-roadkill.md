...
MaxEnt is implemented in Java but can be called in R through package susch as _dismo_ (used here). You'll need to download "maxent.jar" from the Maxent website and place it where [_dismo_](https://cran.r-project.org/web/packages/dismo/index.html) expect it. There are other Jave-free R packages implementing Maxent models, such as [_maxnet_](https://cran.r-project.org/web/packages/maxnet/index.html) (workflows via package [_ENMeval_](https://cran.r-project.org/web/packages/ENMeval/index.html)).
